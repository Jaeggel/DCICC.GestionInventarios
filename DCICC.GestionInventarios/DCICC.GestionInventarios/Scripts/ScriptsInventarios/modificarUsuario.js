...
function cargarRolesTabla() {
    ...
    str += '<thead> <tr> <th>Nombre Usuario</th> <th>Nick</th> <th>Rol</th> <th>Correo</th> <th>Celular</th> <th>Estado</th> <th>Modificar</th> <th>Deshabilitar</th> </tr> </thead>';
    str += '<tbody>';
    for (var i = 0; i < datosRoles.length; i++) {
        var nom = "'" + datosRoles[i].NombresUsuario + "'";
        console.log(nom);

        str += '<tr><td>' + datosRoles[i].NombresUsuario +
            '</td><td>' + datosRoles[i].NickUsuario +
            '</td><td>' + datosRoles[i].CorreoUsuario +
            '</td><td>' + datosRoles[i].TelefonoCelUsuario;

        if (datosRoles[i].HabilitadoUsuario) {
