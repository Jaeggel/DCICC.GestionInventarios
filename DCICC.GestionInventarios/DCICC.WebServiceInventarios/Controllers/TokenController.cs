        ...
        public IActionResult ObtenerTokenTransacciones([FromBody] string infoUsuarioSesion)
        {
            ...
            try
            {
                if (infoUsuarioSesion != null)
                {
                    ...
                    {
                        token = ConfiguracionToken();
                        ConfigBaseDatos.SetCadenaConexion("Server='192.168.0.7';Port=5432;User Id=" + infoUsuario.NickUsuario + ";Password=" + ConfigEncryption.EncriptarValor(infoUsuario.PasswordUsuario) + ";Database=DCICC_BDInventario; CommandTimeout=3020;");
                    }
                    else
                    {
                        return Unauthorized();
